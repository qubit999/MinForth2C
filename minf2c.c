...
void WriteIntro(void) {
    ...
    // on macOS pretend to be Linux/Unix so mf3.h picks POSIX branch
    PR("\n#if defined(__APPLE__)\n");
    PR("#  define __linux__   1\n");
    PR("#  define __unix__    1\n");
    PR("#  define __MACH__    1\n");
    PR("#  undef  __APPLE__\n");
    PR("#endif\n");

    PR("\n#include \"mf3.h\"\n");

    // now in case mf3.h did drop any _open/_close maps, restore POSIX names
    ...
    PR("#undef _creat\n");
    PR("#undef _chdir\n");
    PR("#define _open   open\n");
    PR("#define _close  close\n");
    PR("#define _read   read\n");
    PR("#define _write  write\n");
    PR("#define _lseek  lseek\n");
    PR("#define _unlink unlink\n");
    PR("#define _creat  creat\n");
    PR("#define _chdir  chdir\n");
}
...
void WriteMain(void) {
    if (IsDefined("MAIN")) {
        ...
        PR("\n    if (setjmp(mfabort)==0) {");
        PR("\n        mf%08X(); // %s", hd[hdm].hash, hd[hdm].name);
        PR("\n    }");
        PR("\n    mfexit();      // tidy up once");
        PR("\n    return 0;");
        PR("\n}");
...
int main(int argc, char* argv[]) { // 2-pass transpiler MinForth to C
   ...
   printf("\nextended for macOS (arm64)");
   printf("\nChanged 2025-05-25 by Alexander Slatina (qubit999)");
   printf("\n-----------------------------");
